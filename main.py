...
class App(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        ctk.CTkCheckBox(
            sf,
            text="Запази прозрачност (alpha канал)",
            variable=self._transp_var,
        ).grid(row=1, column=0, columnspan=3, padx=12, pady=(0, 12), sticky="w")
        ...
        self._btn.grid(row=3, column=0, padx=20, pady=10, sticky="ew")

        # Progress row
        pf = ctk.CTkFrame(self)
        pf.grid(row=4, column=0, padx=20, pady=(0, 8), sticky="ew")
        ...
        lf.grid(row=5, column=0, padx=20, pady=(0, 20), sticky="nsew")
    ...
    def _run(self):
        ...
        logging.info(
            f"Start: folder={folder} quality={quality} preserve_transparency={preserve}"
        )
        self._log(
            f"Начало: качество={quality}, "
            f"прозрачност={'запазена' if preserve else 'не запазена'}"
        )
        converted, total = self._convert(folder, quality, preserve)
    ...
    def _convert(self, input_folder: str, quality: int, preserve: bool):
        ...
        for root, _, files in os.walk(old_folder):
            ...
            for filename in files:
                ...
                dst = os.path.join(dest_dir, pathlib.Path(filename).stem + ".webp")
                ...
                try:
                    ...
                    msg = f"  OK: {filename}  ({current}/{total}, {pct*100:.0f}%)"
